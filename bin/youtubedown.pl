...
sub scrape_vimeo_url($$) {
  ...
  if (!check_http_status ($info_url, $http, 0) &&
      $body =~ m/private[+\s]video/si) {  # better error message than 404.
    return scrape_vimeo_moogaloop ($url, $id);
    error ("$id: private video");
  }
  check_http_status ($info_url, $http, 1);
...
# This is the old way (pre-May 2012).
# We need to do this to get at videos that are marked private.
# ...except they seem to have turned this off entirely now. Fuck.
#
sub scrape_vimeo_moogaloop($$) {
  my ($url, $id) = @_;

  my $info_url = "http://vimeo.com/moogaloop/load/clip:$id";
  my $retry_p = 0;
  my ($http, $head, $body) = get_url ($info_url, 
                                      undef, undef, 0, undef, undef, $retry_p);
  if (! check_http_status ($info_url, $http, 0)) {
    error ("$id: private video; moogaloop failed.");
  }
  check_http_status ($info_url, $http, 1);
  ...
  my ($ct, $size);
  ($http, $head, $body) = get_url ($url, undef, undef, 1, undef, undef, 
                                   $retry_p);
  ...
  while ($#ARGV >= 0) {
    ...
    if (m/^--?verbose$/)   { $verbose++; }
      ...
      error ("not a Youtube or Vimeo URL: $_")
        unless (m@^(https?://)?
                   ([a-z]+\.)?
                   (youtube(-nocookie)?\.com|youtu\.be|vimeo\.com)/@six);
      my @P = ($title, $fmt, $_);
      push @urls, \@P;
      $title = undef;
    }
